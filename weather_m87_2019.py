...
def get_data_gfs( fname, site ):
    ...
    elev = np.load('elev_RC/%s_2019_m87_ele.npy'%(site)) # elevation
...
def get_data_LMT():
    ...
    elev = np.loadtxt('elev/elev_LMT_2019.txt') # elevation
    elev_t = np.loadtxt('elev/elev_time.txt') # corresponding time in UTC
...
def get_data_Maunakea():
    ...
    site_g = 'MaunaKea'
    # Read-in SgrA* elevation time series at the site
    elev = np.loadtxt('elev/elev_SMA_2019.txt') # elevation
    elev_t = np.loadtxt('elev/elev_time.txt') # corresponding time in UTC
...
def get_data_ALMA_1():
    ...
    site_g = 'ALMA_1'
    # Read-in SgrA* elevation time series at the site
    elev = np.loadtxt('elev/elev_ALMA_2019.txt') # elevation
    elev_t = np.loadtxt('elev/elev_time.txt') # corresponding time in UTC
...
def get_binned_data(m1,d1,m2,d2,dt):
    ...
    for i in range(len(t)): # day by day loop
        ...
        mjd_2019 = jd_calc( 2019, moda[i][1], moda[i][2] )
        i_elev = int(mjd_2019 - elev[0][0]) # index of elev that corresponds to mjd_2019
...
def plot_percentile(m1,d1,m2,d2,dt,site,GFS=False):
    ...
    f12 = f1.twinx()
    mjd_2019_s = int(jd_calc( 2019, date_min[1], date_min[2] ))
    mjd_2019_e = int(jd_calc( 2019, date_max[1], date_max[2] ))
    for mjd_2019 in range(mjd_2019_s,mjd_2019_e+1):
        i_elev = int(mjd_2019 - elev[0][0])
    ...
    plt.savefig(r'M87_2019_Plot_Percentile_for_'+str(site)+'_during_'+str(m1)+'-'+str(d1)+'_to_'+str(m2)+'-'+str(d2))
...
def plot_probability(m1,d1,m2,d2,dt,val,GFS=False):
    ...
    for site in sites:
        ...
        if avail:
            ...
            elev_temp = np.load('elev_RC/%s_2019_m87_ele.npy'%site)
            da.append(da_temp)
    ...
    for i, m in enumerate(moda):
        ...
        mjd_2019_e = int(jd_calc( 2019, date_max[1], date_max[2] ))
        for mjd_2019 in range(mjd_2019_s,mjd_2019_e+1):
            i_elev = int(mjd_2019 - elev[i][0][0])
            if mjd_2019==mjd_2019_s:
    ...
    plt.savefig(r'M87_2019_Plot_Probability_for_%d-%d_to_%d-%d'%(m1,d1,m2,d2))

"""
def plot_elev(m1,d1,m2,d2,site):

    elev = np.loadtxt('elev/elev_%s_2019.txt'%site)
    elev_t = np.loadtxt('elev/elev_time.txt')

    fig = plt.figure()
    f = fig.add_subplot(111)
    mjd_2019_s = int(jd_calc( 2019, m1, d1 ))
    mjd_2019_e = int(jd_calc( 2019, m2, d2 ))
    for mjd_2019 in range(mjd_2019_s,mjd_2019_e+1):
        i_elev = int(mjd_2019 - elev[0][3])
