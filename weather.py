...
def get_data_gfs( fname, site ):
    ...
    elev = np.loadtxt('elev/elev_%s_2018.txt'%(site)) # elevation
...
def get_binned_data(m1,d1,m2,d2,dt):
    ...
    for i in range(len(t)): # day by day loop
        ...
        i_elev = int(mjd_2018 - elev[0][3]) # index of elev that corresponds to mjd_2018
        ...
        for u_s in np.arange(0.,24.,dt): # for each time bin
            ...
            th = np.interp(u_s+dt/2.,elev_t,elev[i_elev][4:])
            elev_day.append(th)
...
def plot_percentile(m1,d1,m2,d2,dt,site,GFS=False):
    ...
    for mjd_2018 in range(mjd_2018_s,mjd_2018_e+1):
        ...
        f12.plot(elev_t,elev[i_elev][4:],'m-',alpha=0.2)
    f12.set_xlim(xmin,xmax)
    f12.set_ylim(0,90)
    f12.set_ylabel('Elevation [deg]',fontsize=15)
    f12.plot([30,30],[0,1],'m-',label='SgrA* Elevation') # just for legend
    plt.legend(loc='upper right',frameon=False)
    tick_customize(f12, 8.,2.,15.,5.,2.,15.,2.)
    plt.savefig(r'SgrA*_2019_Plot_Percentile_real_data_for_'+str(site)+'_during_'+str(m1)+'-'+str(d1)+'_to_'+str(m2)+'-'+str(d2))
...
def plot_probability(m1,d1,m2,d2,dt,val,GFS=False):
    ...
    for site in sites:
        ...
        if avail:
            ...
            elev_temp = np.loadtxt('elev/elev_%s_2018.txt'%site)
            da.append(da_temp)
    ...
    for i, m in enumerate(moda):
        ...
        mjd_2018_e = int(jd_calc( 2018, date_max[1], date_max[2] ))
        for mjd_2018 in range(mjd_2018_s,mjd_2018_e+1):
            i_elev = int(mjd_2018 - elev[i][0][3])
            if mjd_2018==mjd_2018_s:
                li = elev[i][i_elev][4:]>elev_cutoff
            else:
                li *= elev[i][i_elev][4:]>elev_cutoff
        for mjd_2018 in range(mjd_2018_s,mjd_2018_e+1):
            i_elev = int(mjd_2018 - elev[i][0][3])
            f12.plot(elev_t[li],elev[i][i_elev][4:][li],color=c[i],alpha=0.2)
    ...
    plt.savefig(r'SgrA*_2018_Plot_Probability_real_data_for_'+str(site)+'_during_'+str(m1)+'-'+str(d1)+'_to_'+str(m2)+'-'+str(d2))
