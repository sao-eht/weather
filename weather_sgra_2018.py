...
def get_data_gfs( fname, site ):
    ...
    elev = np.load('elev_RC/%s_2018_sgra_ele.npy'%(site)) # elevation
...
def plot_percentile(m1,d1,m2,d2,dt,site,GFS=False):
    ...
    plt.savefig(r'SgrA*_Plot_Percentile_for_'+str(site)+'_during_'+str(m1)+'-'+str(d1)+'_to_'+str(m2)+'-'+str(d2))
...
def plot_probability(m1,d1,m2,d2,dt,val,GFS=False):
    ...
    for site in sites:
        ...
        if avail:
            ...
            elev_temp = np.load('elev_RC/%s_2018_sgra_ele.npy'%site)
            da.append(da_temp)
    ...
    plt.savefig(r'SgrA*_Plot_Probability_for_%d-%d_to_%d-%d'%(m1,d1,m2,d2))
